...
export const sendPartnerPayoutInvoiceEmail = async ({
  ...
  const calc = calculateTdsAndNet({
    approvedAmount,
    payoutPercentage,
    grossAmount: grossAmount != null ? grossAmount : payoutAmount,
    directAmount: payoutAmount,
    tdsApplicable,
    tdsSection,
    tdsPercentage,
  });

  const finalGross = grossAmount != null ? Number(grossAmount) : calc.grossAmount;
  const finalTdsAmt = tdsAmount != null ? Number(tdsAmount) : calc.tdsAmount;
  ...
  const invoiceHtml = buildPartnerInvoiceHtml({
    ...
    payoutPercentage: payoutPercentage || calc.payoutPercentage,
    tdsApplicable,
    tdsSection: finalTdsSection,
    tdsPercentage: finalTdsRate,
    tdsAmount: finalTdsAmt,
    netAmount: finalNetAmt,
    utrNumber,
    note,
    bankName: bankName || partner.bankName || "",
    accountNumber: accountNumber || partner.accountNumber || "",
